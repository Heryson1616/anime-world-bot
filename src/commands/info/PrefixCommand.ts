...
module.exports = class PrefixCommand extends CommandStructure {
    ...
    async execute(ctx) {
        if (!ctx.args[0]) return this.ket.send({ context: ctx.env, emoji: 'negado', content: `Comando incompleto, a maneira certa de usar é: \`${ctx.user.prefix}${ctx.commandName} prefixo (no máximo 3 caracteres)\`` });
